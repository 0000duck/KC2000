...
{
    public class NegativeZSpriteParameterCalculator : IBloodSpriteParameterCalculator
    {
        private IWallSpriteCalculator _wallSpriteCalculator;
        private double _distanceToWall;
        private ITextureRotater _textureRotater;

        public NegativeZSpriteParameterCalculator(IWallSpriteCalculator wallSpriteCalculator, ITextureRotater textureRotater, double distanceToWall)
        {
        ...
        {
            if (SphereIsAtNegativeZ(centerOfSphere, intersectedElement))
            {
                return CreateSpriteAtNegativeZ(intersectedElement, centerOfSphere, radius);
            }
            return null;
        }

        private WallSpriteAnimationParameters CreateSpriteAtNegativeZ(IWorldElement intersectedElement, Position3D centerOfSphere, double radius)
        {
        ...
        private bool SphereIsAtNegativeZ(Position3D centerOfSphere, IWorldElement intersectedElement)
        {
