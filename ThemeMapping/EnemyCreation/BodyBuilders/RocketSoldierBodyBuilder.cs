...
{
    public class RocketSoldierBodyBuilder : IBodyBuilder
    {
        IDestructibleBody IBodyBuilder.CreateBody(IDestructionObserver collapseStrategy, IDestructionObserver weaponLooser, IDestructionObserver characterRemover, IDestructionObserver bloodExplosionNotifier, IExitWoundObserver exitWoundObserver, double collapseTime, double collapseTimeWithoutLegs, IDucker ducker)
        {
            ...
            //head
            VulnerableBodyPart vulnerableHead = new VulnerableBodyPart(BodyPart.Head, DestructionStrength.RocketSoldierHead);
            IDestructibleBodyPart head = new CenteredHipDependentShape(0.3, 0.15, 0.7, hipHeightProvider, vulnerableHead);
            bodyParts.Add(head);
            //torso
            VulnerableBodyPart vulnerableTorso = new VulnerableBodyPart(BodyPart.Torso, DestructionStrength.RocketSoldierTorso);
            IDestructibleBodyPart torso = new CenteredHipDependentShape(0.6, 0.25, 0.0, hipHeightProvider, vulnerableTorso);
            bodyParts.Add(torso);
            //legs
            VulnerableBodyPart vulnerableLegs = new VulnerableBodyPart(BodyPart.Legs, DestructionStrength.RocketSoldierLegs);
            IDestructibleBodyPart legs = new HipDependentShape(0.25, hipHeightProvider, vulnerableLegs);
            bodyParts.Add(legs);
            //arms
            VulnerableBodyPart vulnerableArms = new VulnerableBodyPart(BodyPart.Arms, DestructionStrength.RocketSoldierArms);
            IDestructibleBodyPart arms = new DegreeBasedShape(0.25, 0.25, 0.3, Degree.Degree_0, 0.25, hipHeightProvider, vulnerableArms);
            bodyParts.Add(arms);
