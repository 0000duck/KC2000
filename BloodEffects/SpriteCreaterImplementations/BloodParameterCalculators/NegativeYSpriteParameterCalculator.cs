...
namespace BloodEffects.SpriteCreaterImplementations.BloodParameterCalculators
{
   ...
   public class NegativeYSpriteParameterCalculator : IBloodSpriteParameterCalculator
    {
        private IWallSpriteCalculator _wallSpriteCalculator;
        private double _distanceToWall;
        private ITextureRotater _textureRotater;

        public NegativeYSpriteParameterCalculator(IWallSpriteCalculator wallSpriteCalculator, ITextureRotater textureRotater, double distanceToWall)
        {
        ...
        {
            if (SphereIsAtNegativeY(centerOfSphere, intersectedElement))
            {
                return CreateSpriteAtNegativeY(intersectedElement, centerOfSphere, radius);
            }
            return null;
        }

        private WallSpriteAnimationParameters CreateSpriteAtNegativeY(IWorldElement intersectedElement, Position3D centerOfSphere, double radius)
        {
            Position3D spriteCenterPosition = new Position3D();

            spriteCenterPosition.PositionX = centerOfSphere.PositionX;
            spriteCenterPosition.PositionY = intersectedElement.Position.PositionZ - _distanceToWall;
        ...
        private bool SphereIsAtNegativeY(Position3D centerOfSphere, IWorldElement intersectedElement)
        {
            return centerOfSphere.PositionZ < intersectedElement.Position.PositionZ;
        }
    }
}
