...
{
    public class NegativeXSpriteParameterCalculator : IBloodSpriteParameterCalculator
    {
        private IWallSpriteCalculator _wallSpriteCalculator;
        private double _distanceToWall;
        private ITextureRotater _textureRotater;

        public NegativeXSpriteParameterCalculator(IWallSpriteCalculator wallSpriteCalculator, ITextureRotater textureRotater, double distanceToWall)
        {
        ...
        {
            if (SphereIsAtNegativeX(centerOfSphere, intersectedElement))
            {
                return CreateSpriteAtNegativeX(intersectedElement, centerOfSphere, radius);
            }
            return null;
        }

        private WallSpriteAnimationParameters CreateSpriteAtNegativeX(IWorldElement intersectedElement, Position3D centerOfSphere, double radius)
        {
        ...
        private bool SphereIsAtNegativeX(Position3D centerOfSphere, IWorldElement intersectedElement)
        {
